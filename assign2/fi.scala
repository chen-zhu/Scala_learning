...
object Images {
  ...
  // !!FILL ME IN
  def lift0[A](a:A): Img[A] = (p:Point) => a
  ...
  // curried:  mul2(2)(3) == 6 
  def lift1[A,B](f: A => B)(img_A: Img[A]): Img[B] = (pt:Point) => f(img_A(pt))
  ...
  // of type C.
  // !!FILL ME IN
  def lift2[A,B,C](f: (A,B) => C)(img_A: Img[A])(img_B: Img[B]): Img[C] = (pt:Point) => f(img_A(pt),img_B(pt))
  ...
  // !!FILL ME IN
  def lift3[A,B,C,D](f: (A,B,C) => D)(img_A: Img[A])(img_B: Img[B])(img_C: Img[C]): Img[D] = (pt:Point) => f(img_A(pt),img_B(pt),img_C(pt))
  ...
  val lerpImgClr = (img_A: ImgGray) => (img_B: ImgClr) => (img_C: ImgClr) => lift3(lerpClr)(img_A)(img_B)(img_C)
  ...
  //http://stackoverflow.com/questions/8000903/what-are-all-the-uses-of-an-underscore-in-scala
  // Partially applied functions _
  val selectImgClr = lift3( 
        (bol: Boolean, col1: Colour, col2: Colour) => if(bol) col1 else col2 
      ) _
  ...
  // Partially applied functions _
  val msk2clr = lift1(
        (bol: Boolean)=> if(bol) Colour(0, 0, 0, 1) else Colour(1, 1, 1, 1) 
      ) _
  ...
  // Partially applied functions _
  val gray2clr = lift1(
        (valu: Value) => Colour(valu,valu,valu,1)
      ) _ 
...
object SpatialTransforms {
  ...
  def translatePt(v: Vector): TransformPt = {
      (pt:Point) => Point(pt.x+v.x, pt.y+v.y)
  }
  ...
  def rotatePt(o: Double): TransformPt = {
      (pt:Point) => Point(pt.x * cos(o) - pt.y * sin(o), pt.y * cos(o) + pt.x * sin(o))
  }
  ...
  def swirlPt(angle: Double): TransformPt = (pt:Point) =>{
      val o = abs(dist(pt)) * (2*PI/angle)
      Point(pt.x*cos(o) - pt.y*sin(o), pt.y*cos(o) + pt.x*sin(o))
  }

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  def polarXf(a: TransformPlr): TransformPt = {
    (pt:Point) => plr2pt( a( pt2plr(pt) ) )
  }
  ...
  def translate[A](v: Vector): (Point => A) => (Point => A) = {
      (img_A: Point => A) => (pt:Point) => img_A(  translatePt(v.neg)(pt) )
  }
  ...
  def scale[A](v: Vector): (Point => A) => (Point => A) = {
      (img_a: Point => A) => (pt:Point) => img_a(  scalePt(v.inverse)(pt) )
  ...
  def rotate[A](a: Double): (Point => A) => (Point => A) = {
    (img_a: Point => A) => (pt:Point) => img_a(  rotatePt(-a)(pt) ) 
  }
  ...
  def swirl[A](a: Double): (Point => A) => (Point => A) = {
    (img_a: Point => A) => (pt:Point) => img_a(  swirlPt(-a)(pt) ) 
  }
...
object Masking {
  ...
  val notM = (img_A: ImgMask) => lift1( (bol: Boolean) => !bol )(img_A)
  ...
  val intersectM = (img_A: ImgMask) => (img_B: ImgMask) => lift2( (b1: Boolean, b2: Boolean) => (b1 && b2) )(img_A)(img_B)
  ...
  val unionM = (img_A: ImgMask) => (img_B: ImgMask) => lift2( (b1: Boolean, b2: Boolean) => (b1 || b2) )(img_A)(img_B)
  ...
  val xorM = (img_A: ImgMask) => (img_B: ImgMask) => lift2((b1: Boolean, b2:Boolean) => ((b1 && !b2) || (!b1 && b2)) )(img_A)(img_B)
