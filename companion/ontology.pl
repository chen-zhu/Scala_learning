...
toisaroot(Begin, Left) :-
	Begin \= 'cellular component',
	is_a(Begin,Type),
	toisaroot(Type,Left2),
	Left = [Begin|Left2].


subconcept(A,B) :- 
	(is_a(A,B); part_of(A,B)).
subconcept(A,B) :- 				% or operator: ; 
	(is_a(B2,B); part_of(B2,B)), %http://stackoverflow.com/questions/13506569/prolog-or-operator-query
	subconcept(A,B2).
