...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  //make a new empty myList with type B, then keep adding to myList to construct map function 
  def map[B](f: A => B): MyList[B] = 
    foldRight(MyList[B]())( 
      (p1:A,p2:MyList[B]) => MyCons(f(p1),p2)
  ) //Or call mapping function f directly on list xs. 

  // take function on head and then append tail
  def flatMap[B](f: A => MyList[B]): MyList[B] = f(x).append(xs.flatMap(f))

  def filter(pred: A => Boolean): MyList[A] = foldRight(MyList[A]())(
      (p1:A, p2:MyList[A]) => 
        if(pred(p1)) MyCons(p1,p2) 
        else p2
  )

  //real from other MyList then add  
  def append(other: MyList[A]): MyList[A] = foldRight(other)( //foldRight all the way 
      (p1:A, p2:MyList[A]) => MyCons(p1,p2) 
  )

  def foldLeft[B](initial: B)(f: (B, A) => B): B = {
    //keep adding to tails here 
    def rec(container: B, list: MyList[A]):B ={
      if(list.isEmpty) container 
      else rec(f(container, list.head), list.tail)
    }
    rec(initial, MyCons(x, xs))
  }

  def foldRight[B](initial: B)(f: (A, B) => B): B = {
    //keep adding to last here 
    def rec(list: MyList[A], container: B):B ={
      //check empty, if empty, return container 
      //else keep append to last using function f: (A,B)=> B ==> 
      if(list.isEmpty) container 
      else rec(list.init, f(list.last, container) ) //talk to Zixia, why list.init? 
    }
    rec(MyCons(x, xs), initial)
  }

  //if length smaller than n, directly return. 
  //else, drop length-n element from begining --> need to double reverse 
  //def take(n: Int): MyList[A] = if(xs.length <= n) MyCons(x,xs) else MyCons(x,xs).reverse.drop(xs.length - n).reverse
  def take(n: Int): MyList[A] = {
    //x+a,b,c,d ==> if n>0, x+(n-1)->in xs
    if (n > 1) MyCons[A](x, xs.take(n-1)) //==> x+list, x-> occupy 1, n need to -1 
    else if (n == 1) MyCons[A](x, MyList[A]()) //if 1, then directly return x 
    else MyList[A]()
    //if n == 0, then need to return empty list case
    

    //val tmp: MyCons[A] = MyCons[A](x, xs) 
    //MyList[A]()
  }
  ...
  def drop(n: Int): MyList[A] = {
    if(n == 0 ) MyCons[A](x, xs)
    else if (n == 1) xs
    else {
      xs.drop(n-1) 
    }

    /*def rec(p1:MyList[A], count:Int): MyList[A] = {
      if(count == 0
      ) p1  //if 0, return everything  
      else rec(p1.tail, count-1) //call tail on MyList, drop the first one
    }
    if(xs.length +1 < n) MyList() //if new constructed mylist size is smaller than n, return empty mylist  
    else rec(MyCons(x,xs),n)*/
  }
  ...
  //reverse, then get tails --> delete the last element, then reverse back 
  //def init: MyList[A] = MyCons(x,xs).reverse.tail.reverse
  def init: MyList[A] = {
    if (xs.isEmpty) MyList[A]() //if only one element, return empty myList  
    else MyCons[A](x, xs.init)
  }

  // do reverse, then get the first, which is the last element in list 
  //def last: A = MyCons(x,xs).reverse.head  
  def last: A = {
    //if MyList is empty, return container x directly 
    if (xs.isEmpty) x //x is the newly append element, if xs is empty, return x directly 
    else xs.last //else, directly call list.last. 
  }
  ...
  def length: Int = xs.length + 1 //constuction need to add 1 ==> for x 
}
...
case class MyNil[A]() extends MyList[A] {
  ...
  def head: A = throw new InvalidOperationException("empty list cannot use head")

  def tail: MyList[A] = throw new InvalidOperationException("empty list cannot use tail")

  def init: MyList[A] = throw new InvalidOperationException("empty list cannot use init")

  def last: A = throw new InvalidOperationException("empty list cannot use last")
